#!/usr/bin/env python3
# main2.py
# Extraction robuste de devis dentaires au format PDF (pdfplumber)
...
def to_float_amount(s: Optional[str]) -> Optional[float]:
    """Convertit '1 520,00' ou '1520.00' en float; renvoie None si impossible."""
    if s is None:
        return None
    s = str(s).strip()
    if not s:
...
def extract_basic_fields(full_text: str) -> Dict[str, Any]:
    ...
    if numero_etablissement_dentiste is None :
        # Regex pour capturer le numéro FINESS
        print("***********")
        print(adresse_patient)
